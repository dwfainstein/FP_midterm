...
plots <- est2 %>%
  ...
  filter(`District` %in% dists) %>% 
  mutate(plots = map(.$data, ~{ #.$data is a potential area that's throwing an error
    ggplot(.x, aes(`Academic Year`, v_2)) +
      geom_errorbar(mapping = aes(ymin = (v_2 - v_se_2), ymax = (v_2 + v_se_2))) + #Trying to generalize from ex.
      geom_point(aes(`Academic Year`, v_2)) +
      guides(fill = "none") +
