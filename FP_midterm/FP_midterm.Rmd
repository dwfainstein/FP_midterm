...
#not sure if we need a combined data frame to do this part--we might be lost in conceptualizing what we're actually plotting and how with models
...
est1 %>%
  mutate(plot = list(data, ~{
    ggplot(aes(n, v)) +
      geom_errorbar(mapping = aes(ymin = (v_2 - est2$v_se_2[7]), ymax = (v_2 +   
                    est2$v_se_2[7]))) +
      geom_point(aes(`Academic Year`, v_2))
