...
plots <- est2 %>%
  group_by(District) %>% 
  nest() %>% #using nest_by() instead of nest() b/c I think we need rowwise function
  filter(District %in% dists) %>% 
  mutate(plots = map2(data, District, ~{ #.$data is a potential area that's throwing an error
