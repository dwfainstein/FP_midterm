...
plots <- est2 %>%
  ...
  mutate(plots = map2(data, District, ~{ 
    ggplot(.x, aes(`Academic Year`, v)) +
      geom_errorbar(mapping = aes(ymin = (v - v_se), ymax = (v + v_se))) +
      ...
      labs(title = glue("Achievement Gap Estimates: {.y}"),
           ...
           x = "School Year",
           y = "Average achievement difference (SD units)",
           caption = glue("Note: District has {n_schools} schools")) + #this doesn't work. I tried reordering the columns, but that won't allow the data to summarize the number of schools. It just categorizes them instead.
...
#To do list:
#add function to change number of schools per district
#use an alternate method to reproduce the plots
#add purple and blue color to points
#add purple and blue color to "white" and "Hispanic/Latino"
````
